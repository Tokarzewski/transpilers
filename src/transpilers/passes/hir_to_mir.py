...
def _default_init_for(ty: Type) -> mir.MirNode:
    ...
    # code.
    return mir.MirRaw(
        snippet="<default-init>",
        ty=UnknownT(hint=f"default init for {type(ty).__name__}"),
    )
...
def _lower_struct(s: hir.HirStruct) -> mir.MirStruct:
    fields = [
        _p(mir.MirParam(name=f.name, ty=_resolve_annotation(f.annotation)), f)
        for f in s.fields
    ]
...
def _lower_function(fn: hir.HirFunction) -> mir.MirFunction:
    params = [
        _p(mir.MirParam(name=p.name, ty=_resolve_annotation(p.annotation)), p)
        for p in fn.params
    ]
...
def _lower_stmt(node: hir.HirNode, env: dict[str, Type]) -> mir.MirNode:
    ...
    if isinstance(node, hir.HirReturn):
        return _p(
            mir.MirReturn(value=_lower_expr(node.value, env) if node.value else None),
            node,
        )
    ...
    if isinstance(node, hir.HirAssign):
        ...
        ty = (
            ann_ty
            if ann_ty is not None and not isinstance(ann_ty, UnknownT)
            else _type_of(value)
        )
        ...
        return _p(
            mir.MirAssign(
                target=node.target, value=value, ty=ty, augmented_op=node.augmented_op
            ),
            node,
        )
...
def _lower_for(node: hir.HirFor, env: dict[str, Type]) -> mir.MirForRange:
    if not (isinstance(node.iter, hir.HirCall) and node.iter.func == "range"):
        raise NotImplementedError(
            "for-loop iter must be range(...) in the initial subset"
        )
    ...
    return _p(
        mir.MirForRange(
            target=node.target, start=start, stop=stop, step=step, body=body
        ),
        node,
    )
...
def _lower_foreach(node: hir.HirForEach, env: dict[str, Type]) -> mir.MirForRange:
    ...
    stop = _p(
        mir.MirCall(
            func="len",
            args=[_p(mir.MirName(name=node.iterable, ty=seq_ty), node)],
            ty=IntT(),
        ),
        node,
    )
...
def _lower_expr(node: hir.HirNode, env: dict[str, Type]) -> mir.MirNode:
    ...
    if isinstance(node, hir.HirName):
        ...
        return _p(
            mir.MirName(
                name=node.name,
                ty=env.get(node.name, UnknownT(hint=f"name {node.name}")),
            ),
            node,
        )
    ...
    if isinstance(node, hir.HirList):
        ...
        elem_ty = (
            _type_of(elements[0]) if elements else UnknownT(hint="empty list literal")
        )
    ...
    if isinstance(node, hir.HirSubscript):
        ...
        ty: Type = (
            value_ty.elem
            if isinstance(value_ty, ListT)
            else UnknownT(hint="subscript on non-list")
        )
    ...
    if isinstance(node, hir.HirFieldAccess):
        ...
        return _p(
            mir.MirFieldAccess(value=_lower_expr(node.value, env), field=node.field),
            node,
        )
    ...
    if isinstance(node, hir.HirStructInit):
        ...
        expected_types = (
            ctor_param_types
            if ctor_param_types is not None
            else [field_types.get(n, UnknownT()) for n in field_names]
        )
...
def _lower_call(node: hir.HirCall, env: dict[str, Type]) -> mir.MirNode:
    ...
    return _p(
        mir.MirCall(func=node.func, args=args, ty=UnknownT(hint=f"call {node.func}")),
        node,
    )
    ...
    if ann.startswith("dict[") and ann.endswith("]"):
        ...
        for i, ch in enumerate(inner):  # split K, V at the top-level comma
            if ch in "[<":
                ...
                return DictT(
                    key=_resolve_annotation(inner[:i].strip()),
